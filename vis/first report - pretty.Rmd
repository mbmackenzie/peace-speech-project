...
```{r}
sampled <- read_csv("data/sampled_articles.csv")

sampled.errors <- sampled %>%
  filter(!is.na(id)) %>%
  group_by(country, year, publisher, total_articles, sampled_articles, articles_found) %>%
  summarise(articles_copied = n()) %>%
  ungroup() %>%
  mutate(found_all_articles = total_articles == articles_found,
         copied_correct_amount = sampled_articles == articles_copied)

sampled.errors %>%
  count(found_all_articles, copied_correct_amount) %>%
  mutate(pct = n / sum(n))

sampled.errors %>%
  filter(articles_copied > sampled_articles)

sampled %>%
  filter(country == "AU", year == 2016, publisher == "9Honey") %>%
  count(id)
  View()
  
sampled %>%
  group_by(id) %>%
  mutate(duplicate_id = n() > 1) %>%
  filter(duplicate_id) %>%
  count(country, year, publisher)

sampled %>% 
  filter(country == "CA", year == 2013, publisher == "Victoria Times Colonist")
```
